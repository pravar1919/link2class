...
from .models import FamilySupport, Like
...
def todays_events_view(request, pk):
    qs = FamilySupport.objects.get(id=pk)
    try:
...
def like_unlike_post_family(request):
    user = request.user.first_name
    # print("********************",user)
    # if not request.method == 'POST' or user.is_authenticated:
    post_id = request.POST.get('post_id')
    # print('post_id ', post_id)
    post_obj = FamilySupport.objects.get(id=post_id)
